...
public class ThreadLvUp extends Thread{
	...
	public void run()
	{
		...
		if(epLv >= 100 && DQM.conf.recalcLvStatus1 == 1)
		{
			epLv = 0;
		...
		for (int i = 0; i < 100; i++)
		{
			...
			if(epLv < 99 && epEXP >= DQM.exp.getNextExp(epLv))
			{
				epLv = epLv + 1;
				...
				ep.worldObj.playSoundAtEntity(ep, "dqm:player.levelup", 1.0F, 1.0F);
	...
	public void lvUpProcess(int lv)
	{
		...
		//DQM.func.debugString("Job:" + epJob);
		switch (epJob)
		{
		...
		//DQM.func.debugString("Job:" + plusHpVal + "/" + plusMpVal + "/" + plusTikaraVal + "/" + plusKasikosaVal);
		...
		ExtendedPlayerProperties.get(this.ep).setMaxHP(DQM.calcPlayerStatus.calcHP(ep));
		ExtendedPlayerProperties.get(this.ep).setMaxMP(DQM.calcPlayerStatus.calcMP(ep));
