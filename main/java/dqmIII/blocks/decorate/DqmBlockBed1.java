...
import dqmIII.blocks.decorate.tileEntity.DqmTileEntityDqmbed;
...
public class DqmBlockBed1 extends BlockContainer
{
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer ep, int par6, float par7, float par8, float par9)
    {
		...
		//外部からの干渉用
		DqmBedEvent event = new DqmBedEvent(ep, 1, 0, new int[]{yadodai});
    	...
    	if(ep.isSneaking())
    	{
    		event = new DqmBedEvent(ep, 1, 1, new int[]{yadodai});
    		event.setCanceled(false);
    		MinecraftForge.EVENT_BUS.post(event);
    		if (event.isCanceled())
    		{
    		...
    		event = new DqmBedEvent(ep, 1, 2, new int[]{yadodai});
    		event.setCanceled(false);
    		MinecraftForge.EVENT_BUS.post(event);
    		if (event.isCanceled())
    		{
    		...
    		event = new DqmBedEvent(ep, 1, 3, new int[]{yadodai});
    		event.setCanceled(false);
    		MinecraftForge.EVENT_BUS.post(event);
    		if (event.isCanceled())
    		{
    		...
    		event = new DqmBedEvent(ep, 1, 4, new int[]{yadodai});
    		event.setCanceled(false);
    		MinecraftForge.EVENT_BUS.post(event);
    		if (event.isCanceled())
    		{
