...
public class DqmWorldGenDungeonOver extends WorldGenerator
{
    ...
    public boolean generate(World world, Random random, int xPos, int yPos, int zPos)
    {
        ...
        if (tileentitymobspawner != null)
        {
        	...
        	if(offY > 40)
        	{
        		tileentitymobspawner.func_145881_a().setEntityName(DQM.modID + "." + DQM.randomMob.getRandomDayName());
        	}else if (offY > 30)
        	{
        		if(random.nextInt(2) == 0)
        		{
        			tileentitymobspawner.func_145881_a().setEntityName(DQM.modID + "." + DQM.randomMob.getRandomNightName());
        		}else
        		{
        			tileentitymobspawner.func_145881_a().setEntityName(DQM.modID + "." + DQM.randomMob.getRandomDayName());
        		}
	        	...
	        	if(world.isAirBlock(offX, offY, offZ))
	        	{
		        	...
		        	if(world.getTileEntity(offX, offY, offZ) instanceof TileEntityChest)
		        	{
			            ...
			            if (tileentitychest != null)
			            {
			            	if(offY > 40)
			            	{
			            		DQM.randomItem.generateChestContentsRank1(random, tileentitychest);
			            	}else
			            	{
			            		DQM.randomItem.generateChestContentsRank2(random, tileentitychest);
			            	}
			            }
		        	}
	        	}
