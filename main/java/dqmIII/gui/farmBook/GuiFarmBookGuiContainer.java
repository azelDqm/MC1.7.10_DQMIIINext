...
public class GuiFarmBookGuiContainer extends GuiContainer {
    private static final ResourceLocation TEXTURE = new ResourceLocation("dqm", "textures/gui/guiFarmBook.png");
    private int page = 0;
	...
	protected void drawGuiContainerBackgroundLayer(float renderPartialTicks, int xMouse, int yMouse)
	{
		...
		if (page > 0)
		{
			seeds = (ItemStack[])DQM.mutationTable.mutationSeeds.get(page - 1);
		}else
		{
		...
		if(page > 0)
		{
			Block[] blocks =(Block[])DQM.mutationTable.mutationBlocks.get(page - 1);
			int[] meta = (int[])DQM.mutationTable.mutationBlockMetas.get(page - 1);
			for(int cnt = 0; cnt < 9; cnt++)
			{
		...
		if(page > 0)
		{
			resultCrop = (Item)DQM.mutationTable.mutationResults.get(page - 1);
		}else
		{
			resultCrop = null;
		}
    ...
    protected void actionPerformed(GuiButton p_146284_1_)
    {
        ...
        if (p_146284_1_.id == 1)
        {
        	...
        	for(int cnt = this.page - 1; cnt >= this.page - DQM.mutationTable.mutationSeeds.size(); cnt--)
        	{
        		int checkPage = cnt - 1;
        		if(checkPage < -1)
    			{
        			checkPage = checkPage + DQM.mutationTable.mutationSeeds.size() + 1;
    			}
	        	...
	        	if(this.page == 0)
	        	{
	        		this.page = DQM.mutationTable.mutationSeeds.size();
	        	}else
	        	{
        ...
        if (p_146284_1_.id == 2)
        {
        	...
        	{
        		int checkPage = cnt - 1;
        		if(checkPage >= DQM.mutationTable.mutationSeeds.size())
    			{
        			checkPage = checkPage - DQM.mutationTable.mutationSeeds.size() - 1;
    			}
        	...
        	/*
        	if(this.page == DQM.mutationTable.mutationSeeds.size())
        	{
        		this.page = 0;
        	}else
        	{
