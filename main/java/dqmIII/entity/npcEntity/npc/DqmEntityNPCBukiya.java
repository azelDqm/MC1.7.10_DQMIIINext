package dqmIII.entity.npcEntity.npc;
...
public class DqmEntityNPCBukiya extends DqmNPCBase
{
    ...
    public void onLivingUpdate()
    {
		...
		*/
        ...
        if(this.itemRandom == -1 || !(this.isPotionActive(DQM.potion.potionCooltimeNPC) || this.isPotionActive(DQM.potion.potionEnableNPC)))
        {
        	//System.out.println("DEBUGLINE:==================================");
        	if(this.worldObj.isDaytime())
        	{
        		this.itemRandom = rand.nextInt(70);
        		this.addPotionEffect(new PotionEffect(DQM.potion.potionEnableNPC.id, 20 * 60 * 10, 0));
        	}else
        	{
        		this.addPotionEffect(new PotionEffect(DQM.potion.potionEnableNPC.id, 20 * 60 * 10, 1));
    ...
    */
    ...
    public boolean interact(EntityPlayer ep)
    {
    	if(!ep.worldObj.isRemote)
    	{
    		...
    		//this.getp
    		if(this.isPotionActive(DQM.potion.potionCooltimeNPC) && flg == 0)
    		{
    		...
    		{
    			//if(this.isPotionActive(DQM.potion.potionEnableNPC))
    			//{
	    			if(flg == 0)
	    			{
	    			...
	    			}else if(flg == 1)
	    			{
    					...
    					tagItem = new ItemStack(DQM.bukiyaPrice.ItemOverWorld(this.itemRandom, this.getActivePotionEffect(DQM.potion.potionEnableNPC).getAmplifier()), 1, 0);
    					itemPrice = DQM.bukiyaPrice.GoldOverWorld(this.itemRandom, this.getActivePotionEffect(DQM.potion.potionEnableNPC).getAmplifier());

    					itemPrice = DQM.calcPlayerStatus.calcShoninGold(itemPrice, ep);
    					...
    					if(ExtendedPlayerProperties.get(ep).getBukiyaItem() != null && tagItem.getItem() != null && tagItem.getItem() == ExtendedPlayerProperties.get(ep).getBukiyaItem() && ep.isSneaking())
    					{
    						...
    						if(epGold >= itemPrice)
    						{
    		    				...
    		    				{
    		    					int durLeast = this.getActivePotionEffect(DQM.potion.potionEnableNPC).getDuration();

    		    					ep.addChatMessage(new ChatComponentTranslation("msg.Bukiya.messages.3.txt",new Object[] {}));

    		    					this.addPotionEffect(new PotionEffect(DQM.potion.potionCooltimeNPC.id, durLeast, 0));
    		    					this.removePotionEffect(DQM.potion.potionEnableNPC.id);
    		    					//this.addPotionEffect(new PotionEffect(DQM.potion.potionEnableNPC.id, 20 * 60 * 10, 1));
    		...
    		}if(flg == 1)
    		{
    			...
    			int syuuriPrice = DQM.syuuriPrice.Gold(its);
    			if(syuuriPrice >= 0)
    			{
    				if(!ep.isSneaking())
    				{
