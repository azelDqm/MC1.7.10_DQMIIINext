            	...
            	for(int cnt = 0; cnt < list.size(); cnt++)
            	{
        			if(this.attacker.mobAI.getHeavyFire() > 0)
        			{
        				if(list.get(cnt) instanceof EntityPlayer)
        				{
        					...
        					if(!ep.isPotionActive(DQM.potion.debuffHeavyFire.id));
        					{
        						ep.addPotionEffect(new PotionEffect(DQM.potion.debuffHeavyFire.id, 100, this.attacker.mobAI.getHeavyFire()));
        					}
        				}
        			}
