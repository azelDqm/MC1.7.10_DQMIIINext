...
public class DqmEntityHitokuikibako extends DqmMobBaseDay
{
	...
	protected void applyEntityAttributes()
	{
		...
		this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue(DQM.funcMob.getCalcSPEED(EnumDqmMonster.HITOKUIKIBAKO.getSPEED()));
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(DQM.funcMob.getCalcHP(EnumDqmMonster.HITOKUIKIBAKO.getHP()));
        this.getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(DQM.funcMob.getCalcPW(EnumDqmMonster.HITOKUIKIBAKO.getPW()));
	}
    ...
    protected void dropFewItems(boolean par1, int par2)
    {
        ...
        {
            this.dropItem(DQM.weapons.itemDokubari, 1);
        }
        if (DQM.funcMob.getCalcDROP(15, 1))
        {
            this.dropItem(DQM.ingots.itemTokinosuisyou, 1);
        }
        if (DQM.funcMob.getCalcDROP(10, 1))
        {
            this.dropItem(DQM.miscs.itemMajuunokawa, 1);
        }
        if (DQM.funcMob.getCalcDROP(5, 1))
        {
            this.dropItem(DQM.ingots.itemTekkouseki, 1);
        }
        if (DQM.funcMob.getCalcDROP(1, 1))
        {
            this.dropItem(DQM.seeds.itemYakusou, 1);
        }
