package dqmIII.entity.magicEntity.magic;
                ...
                if (this.ticksInGround > limit)
                {
                	if(boxHit > 0)
                	{
        	            List list2 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this,
        	            		this.boundingBox.addCoord(this.motionX, this.motionY, this.motionZ).expand((double)boxHit, (double)boxHit, (double)boxHit));
                            	...
                            	if(flg < this.rate)
                            	{
                            		if(target2 instanceof EntityLivingBase)
                            		{
                            			...
                            			if (ebl2.attackEntityFrom(damagesource2, (float)i1))
        	                            {
        	                                ...
        	                                //ここでヒット時の効果音がなる
        	                                this.playSound("dqm:mob.dokubari", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
        ...
        else//埋まってない時。速度の更新。
        {
                        ...
                        else
                        {
                        	...
                        	if(flg < this.rate)
                        	{
	                        	...
	                        	if (target.entityHit.attackEntityFrom(damagesource, (float)i1))
	                            {
	                                ...
	                                //ここでヒット時の効果音がなる
	                                this.playSound("dqm:mob.hyado", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
                            	...
                            	if(boxHit > 0)
                            	{
	                	            ...
	                	            if (list2 != null && !list2.isEmpty())
	                	            {
	                	            	for (int n = 0 ; n < list2.size() ; n++)
	                	            	{
	                                		...
	                                		if(target2 instanceof EntityLiving)
	                                		{
	                                			EntityLiving elv2 = (EntityLiving)target2;
	                                			pe = elv2.getActivePotionEffect(DQM.potion.buffMahokanta);
	                                		}
	                                		...
	                                		if(pe != null && this.shootingEntity != null && this.shootingEntity instanceof EntityLiving)
	                                		{
		                                    	...
		                                    	if(flg < this.rate)
		                                    	{
		                                    		...
		                                    		{
		                                    			EntityLivingBase ebl2 = (EntityLivingBase)elv;
		                                    			i1 = (int)ebl2.getMaxHealth();
		                                    			//target.entityHit.attackEntityFrom(damagesource, (float)i1);

		                                    			if (ebl2.attackEntityFrom(damagesource, (float)i1))
		                	                            {
		                	                        		//ダメージを与えることに成功したら以下の処理を行う
		                	                                if (ebl2 instanceof EntityLivingBase)
		                	                                {
            ...
            if (movingobjectposition != null)//blockのみ
            {
                if (!this.isPenetrateBlock())  //エンティティには当たってない。ブロックに当たった。
                {
                    ...
                    this.playSound("dqm:mob.fire", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
                    this.inGround = true;
